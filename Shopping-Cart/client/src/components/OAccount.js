...
const Account = () => {
    ...
    const ownerItems = async (req, res) => {
        try {
            const email = odta.email;
            const res = await fetch("/getowneritems", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json"
                },
    ...
    return (
        <>
                        ...
                        <form method="POST">
                            ...
                            <div className="form-group">
                                <label className="name-field">
                                    ...
                                    <input type="text" name="link" className="exampleInputText" placeholder="Paste image link " autoComplete="off"
                                        value={item.link}
                                        onChange={inputEvent}
                                    />
                                </label>
            ...
            <div className="text-center"><h3>Your Items</h3></div>
            <div className="container shop-div">
                {
                    product.map((currEle, Ind) => {
                        return (
                            <Itemcard
                                id={product[Ind]._id}
                                name={currEle.name}
